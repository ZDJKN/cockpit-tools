...
本 Fork 增加了独立的“企业账号池”页面，用于验证以下产品闭环：Mock／钉钉双身份、安装级 Windows 设备 ID、系统浏览器登录交接、单账号单设备排他租约、FIFO 排队、低额度确认换号和主动释放。
...
打开 `http://127.0.0.1:1420`，进入“企业账号池”即可执行完整 Mock 流程。客户端会自动生成并复用设备 ID，不再要求用户手工填写；企业身份和短期会话 Token 只保存在当前进程内存，刷新应用后需要重新登录。

钉钉模式会创建一次性登录任务，用系统浏览器完成 OAuth，再由 cockpit-tools 安全轮询并自动绑定设备。没有管理员参数时由协议测试覆盖；填入正式参数后客户端不需要改代码。

> 当前里程碑仍不保存或下发真实 ChatGPT/Codex 凭据，也不修改 `auth.json` 或 Windows Credential Manager。真实凭据投影、设备密钥和合规评估属于后续阶段。
